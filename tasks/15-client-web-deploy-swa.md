...
`func start`, or (b) a Tauri `.app` bundle that ships the same static
export but is macOS-only, unsigned, and intended for hand-delivery to
testers (see spec 40). Neither is suitable when we want to point a
collaborator at a URL and have them try the wrap flow against the real
deployed backend.
...
`var.static_web_app_name` to `infra/variables.tf` and the SWA host into
`var.wrap_allowed_origins` automatically:
...
### 7. `test/unit/static-export-invariants.test.ts`

Static-analysis test, mirroring the pattern in
`test/unit/privacy-invariants.test.ts`. Asserts:

- `next.config.mjs` declares `output: 'export'` unconditionally
  (one-line regex check).
- After `pnpm build`, `out/_next/server` does not exist.
- No file under `out/` contains the string `"use server"`.

The last two run against a cached fixture if `out/` isn't present —
the CI workflow runs the full build and re-runs the test, so a missing
`out/` in local `pnpm test` doesn't fail. If spec 40 (Tauri shell
bootstrap) ships first and creates `tauri-invariants.test.ts` with
overlapping assertions, fold those checks into this file and rename
it; the contract is the same on both shells.
...
- **`pnpm build`** produces `out/` with no `_next/server` directory and
  no file containing `"use server"`. (Asserted by piece 7's invariant
  test and re-asserted by the deploy workflow's "Verify static export"
  step.)
- **`pnpm test`** — `test/unit/static-export-invariants.test.ts` passes;
  existing privacy invariants still hold; `src/app/api/` still absent.
...
- Touches: `next.config.mjs`, `package.json` (repoint `start` to
  ...
  `test/unit/static-export-invariants.test.ts` (new),
  `README.md` (add "Hosting (web)" section),
  `ARCHITECTURE.md` (name SWA as client target),
  `CLAUDE.md` (file map + universal export contract).
- Pairs with **spec 14** (server deploy artifact, done): without that
  spec the Function App has no code to receive CORS-allowed requests.
- Pairs with **spec 40** (Tauri shell bootstrap, shaped): both rely on
  `out/` being a clean static export. Either order is fine. If spec
  40 lands first, fold its `tauri-invariants.test.ts` into the
  static-export-invariants file rather than keeping two near-duplicate
  tests. If this spec lands first, spec 40 picks up the universal
  export contract and drops its `TAURI=1` conditional handling for
  `next.config.mjs`.
